...
// L3Hpwm->period(periodDC);  //Period of pwm = 0.01seconds (10ms)
...
void readPIrunMotor(){
    ...
    if (intState != intStateOld) {
        ...
        if (modulus(currentRPSValue) <= thresholdRPS) {
            //Turn off first
            ...
            delete L1Hdigi; //default pin type declared as DigitalOut upon initialisation
            // PwmOut *L1Hpwm = new PwmOut(L1Hpin);
            // L1Hpwm->period(periodDC);  //Period of pwm = 0.01seconds (10ms)
            ...
            delete L2Hdigi;
            // PwmOut *L2Hpwm = new PwmOut(L2Hpin);
            // L2Hpwm->period(periodDC);  //Period of pwm = 0.01seconds (10ms)
